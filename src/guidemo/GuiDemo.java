...
public class GuiDemo extends JFrame{
	...
	public GuiDemo() {
		...
		drawPanel.getTextItem().setText(
				"Punem textul nostru Big bugs have little bugs\n" +
				"      Upon their backs to bite 'em,\n" +
				"And little bugs have littler bugs,\n" +
				"      And so it goes, ad infinitum."
			);
		drawPanel.getTextItem().setFontSize(30);
		drawPanel.getTextItem().setJustify(TextItem.LEFT);
		drawPanel.setBackgroundImage(Util.getImageResource("resources/images/default.jpeg"));
...
//		setJMenuBar(menuBar);
		...
		/**
		 * Once you have a method to create the menu, 
		 * find the place in the constructor of the GuiDemo class where the menu bar is created. 
		...
		JMenu stamper = iconSupport.createMenu();
		menuBar.add(stamper);
		//we add on menu bar the stamper.
		setJMenuBar(menuBar);
	...
	private JMenu makeBackgroundMenu() {
		...
		menu.add(new ChooseBackgroundAction("Default"));
		menu.addSeparator();
	...
	/**
	 * Creation of toolbar.
	 * Make a JToolBar that duplicates the functionality of 
	 * the "Background" menu, except for the JCheckBox that 
	...
	private JToolBar makeToolbar() {
		...
		toolbar.add(new ChooseBackgroundAction("Default"));
		toolbar.addSeparator();
		    		...
		    		try {
		    			...
		    			if (fileName.endsWith(".png"))
		    				...
		    				JOptionPane.showMessageDialog(drawPanel,
		    						"The output file name must end with\n.png or .jpeg.");
		    				return;
		    			}
    ...
    private class ChooseBackgroundAction extends AbstractAction {
    	...
    	ChooseBackgroundAction(String text) {
    		...
    		if (text.equals("Color...")){
    			/**
    			 * Create a BufferedImage, use it to make an ImageIcon, 
    			 * and use that icon for the "Color..." action.
    			 * To make the image, create a new BufferedImage with width 32, 
    			 * height 32, and type BufferedImage.TYPE_INT_RGB. 
    			 */
    			...
    			graphics.fillRect(0,  0,  10,  32);
    			graphics.setColor(Color.GREEN);
    			graphics.fillRect(10, 0, 21, 32);
    			graphics.setColor(Color.BLUE);
    			graphics.fillRect(21,  0,  32,  32);
    			graphics.dispose();
    			putValue(Action.SMALL_ICON, new ImageIcon(icon));
    		}
    		else if (text.equals("Custom...")) {
    			/**
				* Use the resource "resources/action_icons/fileopen.png" 
				* to make an icon for the "Custom..." action. 
				*/	
